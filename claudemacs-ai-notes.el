...
;;;; Org-roam-worktree Integration

(defun claudemacs-worktree-hook (worktree-path node-id branch-name)
  "Hook function for org-roam-worktree integration.
Called after a worktree is created.
WORKTREE-PATH is the path to the new worktree.
NODE-ID is the org-roam node ID.
BRANCH-NAME is the git branch name.
...
Spawns a claudemacs session in the worktree and sends the task context."
  ...
  (let* ((buffer-name (claudemacs-spawn-agent worktree-path branch-name
    ...
    (run-with-timer
     3 nil
     (lambda (buf-name content nid wpath)
       (when-let ((buffer (get-buffer buf-name)))
         ;; Send task context to Claude
         ...
         (claudemacs-worktree--update-node-session nid buf-name)))
     buffer-name task-content node-id worktree-path)))

(defun claudemacs-worktree-start-hook (worktree-path node-id)
  "Hook function called when starting/resuming a worktree task.
WORKTREE-PATH is the worktree directory.
NODE-ID is the org-roam node ID.

If a claudemacs session already exists for this worktree, focus it.
Otherwise, spawn a new one."
  (require 'claudemacs)
  (let* ((expanded-path (expand-file-name worktree-path))
         (existing-buffer (claudemacs-worktree--find-session-buffer expanded-path)))
    (if existing-buffer
        ;; Session exists, just focus it
        (progn
          (display-buffer existing-buffer)
          (select-window (get-buffer-window existing-buffer))
          (message "Resumed claudemacs session for worktree"))
      ;; No session, the create hook will handle spawning
      nil)))

(defun claudemacs-worktree--find-session-buffer (worktree-path)
  "Find an existing claudemacs buffer for WORKTREE-PATH."
  (let ((expanded-path (expand-file-name worktree-path)))
    (cl-find-if
     (lambda (buf)
       (and (string-match-p "^\\*claudemacs:" (buffer-name buf))
            (with-current-buffer buf
              (and (boundp 'claudemacs--cwd)
                   (string= (expand-file-name claudemacs--cwd) expanded-path)))))
     (buffer-list))))

(defun claudemacs-worktree--get-node-content (node-id)
  "Get the content of the org-roam node with NODE-ID."
  (when-let ((node (org-roam-node-from-id node-id)))
    (let ((file (org-roam-node-file node)))
      (with-temp-buffer
        (insert-file-contents file)
        (goto-char (point-min))
        ;; Skip properties drawer and get the content
        (when (re-search-forward "^#\\+filetags:" nil t)
          (forward-line 1)
          (string-trim (buffer-substring-no-properties (point) (point-max))))))))

(defun claudemacs-worktree--update-node-session (node-id buffer-name)
  "Update the org-roam node NODE-ID with claudemacs session info.
BUFFER-NAME is the claudemacs buffer name."
  (when-let ((node (org-roam-node-from-id node-id)))
    (let ((file (org-roam-node-file node)))
      (with-current-buffer (find-file-noselect file)
        (save-excursion
          (goto-char (point-min))
          ;; Get session ID from the claudemacs buffer if available
          (when-let ((claude-buf (get-buffer buffer-name)))
            (let ((session-id (with-current-buffer claude-buf
                                (when (boundp 'claudemacs--session-id)
                                  claudemacs--session-id))))
              (when session-id
                (org-set-property "SESSION_ID" session-id))))
          (org-set-property "CLAUDEMACS_BUFFER" buffer-name)
          (save-buffer))))))

(defun claudemacs-worktree-setup-hooks ()
  "Set up claudemacs hooks for org-roam-worktree integration.
Call this after both claudemacs and org-roam-worktree are loaded."
  (when (featurep 'org-roam-worktree)
...
(with-eval-after-load 'org-roam-worktree
  (claudemacs-worktree-setup-hooks))
